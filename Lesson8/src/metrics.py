...
def money_precision_at_k(recommended_list, bought_list, df_price, k=5):
    ...
    for item in recommended_list:
        try:
            s = df_price.loc[df_price['item_id'] == item]['price'].values
        except:
            s = 0
        prices_recommended.append(s)
...
#      recommended_list]
    ...
    prices = np.dot(flags, prices_recommended) # flags @ prices_recommended
    precision = prices.sum() / prices_recommended.sum() #if (len(prices) * len(prices_recommended) > 0) else 0
